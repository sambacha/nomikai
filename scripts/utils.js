...
async function getDeployedMoloch() {
  ...
  if (!molochAddress) {
    console.error(
      `Please, set the DAO's address in buidler.config.js's networks.${buidlerArguments.network}.deployedContracts.moloch`,
    );
    return;
  }
...
async function getDeployedPool() {
  ...
  if (!poolAddress) {
    console.error(
      `Please, set the Pool's address in buidler.config.js's networks.${buidlerArguments.network}.deployedContracts.pool`,
    );
    return;
  }
